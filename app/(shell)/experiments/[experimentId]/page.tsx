...
export default function ExperimentDetailPage() {
  ...
  function runReplayHref(run: ExperimentRun) {
    const promptPacket = packets.find((packet) => packet.runId === run.id && packet.type === 'prompt');
    const prompt = promptPacket ? (typeof promptPacket.input === 'string' ? promptPacket.input : JSON.stringify(promptPacket.input)) : '';
    const params = new URLSearchParams({ experimentId, model: run.model, prompt });
    const executionMode = promptPacket?.metadata?.executionMode;
    if (executionMode === 'direct' || executionMode === 'python') params.set('executionMode', executionMode);
    if (typeof run.parameters.temperature === 'number') params.set('temperature', String(run.parameters.temperature));
    if (run.parameters.systemPrompt) params.set('systemPrompt', run.parameters.systemPrompt);
  ...
  return <div className="space-y-6 animate-in fade-in duration-500">
    ...
    {experiment ? <>
      ...
      <section className="rounded-xl border border-border/60 bg-card/40 overflow-hidden"><div className="px-5 py-4 border-b border-border/50"><h2 className="text-sm font-semibold">Recorded runs</h2><p className="text-xs text-muted-foreground mt-1">Each execution links its prompt and completion packets. Replay restores the recorded target and settings before you choose to execute again.</p></div><div className="divide-y divide-border/40">{runs.map((run) => <div key={run.id} className="px-5 py-3 flex flex-wrap items-center justify-between gap-3 text-xs"><div><p className="font-mono">{run.model} <span className="text-muted-foreground">· {run.provider}</span></p><p className="text-[10px] text-muted-foreground mt-1">{new Date(run.startedAt).toLocaleString()} · {run.metrics.totalPackets} packets · {run.metrics.avgLatencyMs}ms</p></div><div className="flex items-center gap-2"><Badge variant={run.status === 'completed' ? 'amber' : 'purple'} className="font-mono">{run.status}</Badge><Link href={runReplayHref(run)}><Button variant="outline" size="sm" className="h-7 text-[10px]">Replay run</Button></Link></div></div>)}{runs.length === 0 && <p className="p-5 text-xs text-muted-foreground">This experiment has no formal run records yet. Older packets remain inspectable below.</p>}</div></section>
