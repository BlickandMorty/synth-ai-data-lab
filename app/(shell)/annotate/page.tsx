...
import { Check, Scale, Tag, TriangleAlert } from 'lucide-react';
...
export default function AnnotatePage() {
  ...
  useEffect(() => {
    fetch('/api/packets?limit=300').then((response) => response.json()).then((data) => {
      ...
      const first = list.find((packet: DataPacket) => packet.id === requestedLeft) || list[0];
      const second = list.find((packet: DataPacket) => packet.id === requestedRight) || list.find((packet: DataPacket) => packet.id !== first?.id && promptValue(packet) === promptValue(first)) || list.find((packet: DataPacket) => packet.id !== first?.id);
  ...
  async function save() {
    ...
    if (data.success) setSaved(true); else setError(data.error || 'Could not save annotation.');
  }

  return <div className="space-y-6 animate-in fade-in duration-500">
    <div className="border-b border-border/40 pb-6"><div className="flex items-center gap-2 text-xs font-mono text-[#C4956A] mb-2 uppercase tracking-widest"><Tag className="w-3.5 h-3.5" /> Human review layer</div><h1 className="text-3xl text-foreground font-bold tracking-tight">ANNOTATION STUDIO</h1><p className="text-muted-foreground text-sm mt-2 max-w-3xl">A preference pair is only useful when it compares two real completions for the same prompt. Scores and notes are separate review records, so original model output remains intact.</p></div>
